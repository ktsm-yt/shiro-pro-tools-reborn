import type { BuffMode, Stat } from '../types';
...
export const patterns: ParsedPattern[] = [
  ...
  { stat: 'cost', mode: 'flat_sum', regex: /城娘.*?撃破.*?気(?:が)?\s*(\d+)/, costType: 'ally_defeat' },
  ...
  // 10. 攻撃回数
  { stat: 'attack_count', mode: 'flat_sum', regex: /攻撃回数(?:が)?\s*(\d+)増加/ },
];
