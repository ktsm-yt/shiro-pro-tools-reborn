# ShiroPro Tools (Reborn) - 統合設計書

**Version**: 2.0  
**Last Updated**: 2025-12-07

## 1. プロジェクト全体像

### 1.1 目的
城プロREの編成管理、バフ可視化、ダメージ計算を行うWebアプリ。
旧ツール（buffParser.js）の資産を活かしつつ、TypeScript + React で堅牢に再構築。

### 1.2 コア原則
1. **Core/UI分離**: 計算ロジックはReact非依存の純粋関数
2. **型安全**: すべてのデータにTypeScript型定義
3. **テスタブル**: Vitestでロジックを徹底テスト
4. **段階的移植**: 旧ツールのロジックを少しずつ移植
5. **実用性重視**: 実際のゲームプレイに基づいた設計

---

## 2. データモデル設計

### 2.1 Reborn 型定義（完全版）

#### Stat（バフ種別）

バフの種別を目的別に4つのカテゴリに分類します。この分類により、コードの見通しが良くなり、計算ロジックの責務が明確になります。

```typescript
// ========================================
// 味方バフ系（編成パズル＋ダメージ計算）
// ========================================
type AllyBuffStat =
  | 'attack'              // 攻撃力
  | 'defense'             // 防御力
  | 'range'               // 射程
  | 'attack_speed'        // 攻撃速度（旧版の「速度」）
  | 'attack_gap'          // 攻撃隙（旧版の「隙」）
  | 'recovery'            // 回復力
  | 'cost'                // 自然気（40%/70%）
  | 'cost_gradual'        // 徐々気（時間経過で増加）
  | 'cost_giant'          // 気軽減（巨大化消費気軽減）
  | 'cost_strategy'       // 計略消費気軽減
  | 'cost_enemy_defeat'   // 気牛（敵を倒すと気増加）
  | 'cost_defeat_bonus'   // 気ノビ（撃破時の獲得気増加）
  | 'strategy_cooldown'   // 計略再使用時間
  | 'target_count'        // 攻撃対象数
  | 'attack_count';       // 攻撃回数

// ========================================
// ダメージ計算専用バフ（乗算系）
// ========================================
type DamageMultiplierStat =
  | 'damage_dealt'        // 与ダメ
  | 'give_damage'         // 与えるダメージ（別計算式）
  | 'damage_taken'        // 被ダメ（敵側）
  | 'damage_recovery'     // 与ダメ回復
  | 'critical_bonus';     // 直撃ボーナス

// ========================================
// 敵デバフ系（ダメージ計算専用）
// ========================================
type EnemyDebuffStat =
  | 'enemy_attack'                      // 敵の攻撃力低下
  | 'enemy_defense'                     // 敵の防御力低下
  | 'enemy_defense_ignore_percent'      // 割合防御無視（○%）
  | 'enemy_defense_ignore_complete'     // 完全防御無視
  | 'enemy_movement'                    // 敵の移動速度低下
  | 'enemy_retreat'                     // 敵後退（マス数）
  | 'enemy_range'                       // 敵の射程低下
  | 'enemy_damage_dealt'                // 敵の与ダメ低下（防御貢献）
  | 'enemy_damage_taken';               // 敵の被ダメ上昇（攻撃貢献）

// ========================================
// 特殊バフ系
// ========================================
type SpecialBuffStat =
  | 'inspire';            // 鼓舞（専用計算）

// ========================================
// Wikiパース用レガシー型（baseStats格納用）
// ========================================
type LegacyStat =
  | 'hp'                  // 耐久
  | 'cooldown'            // クールダウン
  | 'movement_speed'      // 移動速度
  | 'knockback'           // ノックバック
  | 'ki_gain'             // 気獲得
  | 'damage_drain'        // ダメージ吸収
  | 'ignore_defense';     // 防御無視

type Stat = AllyBuffStat | DamageMultiplierStat | EnemyDebuffStat | SpecialBuffStat | LegacyStat;
```

#### BuffMode（適用方式）

```typescript
type BuffMode =
  | 'percent_max'        // 割合バフ（+30%）
  | 'flat_sum'           // 固定値加算（+50）
  | 'percent_reduction'  // 割合減少（隙短縮30%, 気軽減20%等）
  | 'absolute_set';      // 絶対値設定（直撃ボーナスが○%に上昇）
```

#### Target（バフの範囲）

**重要な設計原則**: targetは「範囲」を表し、ConditionTagは「その範囲内での絞り込み」を表します。

```typescript
type Target = 
  | 'self'           // 自分自身
  | 'ally'           // 対象指定（1体）
  | 'range'          // 射程内
  | 'all'            // 全体
  | 'field'          // フィールド全体（気など）
  | 'out_of_range';  // 射程外
```

**各Targetの説明**:

| Target | 意味 | 例 | 備考 |
|--------|------|-----|------|
| `self` | 自分自身 | 「自身の攻撃が30%上昇」 | 最も基本的なバフ |
| `ally` | 対象指定（1体） | 「対象の攻撃が50%上昇」 | 計略で特定の味方を選択 |
| `range` | 射程内 | 「射程内の味方の攻撃が20%上昇」 | 最も一般的な範囲バフ |
| `all` | 全体 | 「全ての城娘の攻撃が15%上昇」 | 編成特技や一部の戦略 |
| `field` | フィールド全体 | 「自然に気が5増加」 | 気など、個別キャラではなく編成全体のリソース |
| `out_of_range` | 射程外 | 「射程外の味方の防御が30%上昇」 | 甲府城のような特殊バフ |

**ConditionTagとの組み合わせ例**:

```typescript
// 射程内の近接城娘の攻撃が50%上昇
{
  stat: 'attack',
  target: 'range',           // 範囲：射程内
  conditionTags: ['melee'],  // 絞り込み：近接のみ
  value: 50
}

// 全ての水属性城娘の防御が30%上昇
{
  stat: 'defense',
  target: 'all',             // 範囲：全体
  conditionTags: ['water'],  // 絞り込み：水属性のみ
  value: 30
}
```

#### サブタイプ定義

気バフは獲得方法や用途によって細かく分類されます。

```typescript
type CostBuffType = 
  | 'natural'        // 自然回復（秒間○増加）
  | 'enemy_defeat'   // 敵撃破時（牛）
  | 'ally_defeat'    // 城娘撃破時（ノビ）
  | 'gradual'        // 徐々に増加
  | 'strategy_use'   // 計略使用時
  | 'strategy_cost'  // 計略消費気減少（※固定値が多い）
  | 'giant_cost';    // 巨大化気軽減

type DynamicBuffType = 
  | 'per_enemy_in_range'      // 射程内敵1体毎
  | 'per_ally_in_range'       // 射程内味方1体毎
  | 'per_ally_other'          // 味方1体毎（自身除く）
  | 'per_ambush_deployed'     // 配置された伏兵1体毎
  | 'per_enemy_defeated'      // 撃破した敵1体毎
  | 'per_specific_attribute'  // 特定属性の城娘1体毎
  | 'per_specific_weapon';    // 特定武器種の城娘1体毎
```

#### Buff インターフェース（完全版）

```typescript
interface Buff {
  id: string;
  stat: Stat;
  mode: BuffMode;
  value: number;
  source: 'self_skill' | 'ally_skill' | 'strategy' | 'formation_skill';
  target: Target;
  conditionTags?: ConditionTag[];
  isActive: boolean;
  
  // 効果重複関連フラグ
  isDuplicate?: boolean;              // 効果重複（別途計算される）
  isExplicitlyNonDuplicate?: boolean; // 明示的な重複不可（「同種効果の重複無し」等）
  nonStacking?: boolean;              // 重複なし（最大値のみ適用）
  stackPenalty?: number;              // 重複時効果減少（0-100の減少率）
  stackable?: boolean;                // スタック可能（敵撃破毎に等）
  maxStacks?: number;                 // 最大スタック数
  currentStacks?: number;             // 現在のスタック数（実行時に使用）

  // 優先度（同一statで複数Buffがある場合の選択基準）
  priority?: 'low' | 'normal' | 'high';
  
  // 動的バフ関連
  isDynamic?: boolean;
  dynamicType?: DynamicBuffType;
  dynamicCategory?: 'formation' | 'combat_situation';
  unitValue?: number;                // 単位あたりの効果値
  dynamicParameter?: string;         // 動的パラメータの説明
  
  // 伏兵依存フラグ
  requiresAmbush?: boolean;   // 伏兵が配置されている必要があるバフ
  
  // cost専用フィールド
  costType?: CostBuffType;
  
  // inspire専用フィールド
  inspireSourceStat?: Stat;   // 参照元（'attack', 'defense'等）
  
  // 手動編集サポート
  confidence?: 'certain' | 'inferred' | 'uncertain';
  inferenceReason?: string;
  
  // 補足情報
  note?: string;              // 効果時間、範囲サイズなどの追加情報
}
```

### 2.2 旧ツール（buffParser.js）の出力形式

旧buffParser.jsは以下の形式でバフ情報を出力します。

```javascript
// parseSkillLine の出力例
{
  type: '攻撃割合',      // → Stat へ変換
  target: '自身',        // → target へ変換
  value: 30,
  unit: '+%',           // → BuffMode へ変換
  isSpecial: false,
  hasCondition: false,
  note: ''
}
```

### 2.3 型変換マッピング（要約版）

詳細なマッピングテーブルは `TYPE_CONVERSION_MAPPING.md` を参照してください。ここでは主要なパターンのみ記載します。

#### 攻撃・防御系

| 旧type | 旧unit | → Reborn Stat | → BuffMode | 備考 |
|--------|--------|---------------|------------|------|
| 攻撃割合 | `+%` | `attack` | `percent_max` | |
| 攻撃割合 | `×` | `attack` | `percent_max` | 倍率→%変換 |
| 攻撃固定 | `+` | `attack` | `flat_sum` | |
| 防御割合 | `+%` | `defense` | `percent_max` | |
| 防御固定 | `+` | `defense` | `flat_sum` | |
| 攻撃デバフ割合 | `+%` | `enemy_attack` | `percent_max` | |
| 防御デバフ割合 | `+%` | `enemy_defense` | `percent_max` | |
| 防御無視 | - | `enemy_defense_ignore_complete` | `flat_sum` | value=1 |
| 防御無視（割合） | `%` | `enemy_defense_ignore_percent` | `percent_max` | |

#### ダメージ系

| 旧type | 旧unit | → Reborn Stat | → BuffMode | 備考 |
|--------|--------|---------------|------------|------|
| 与ダメ | `+%/×` | `damage_dealt` | `percent_max` | |
| 与えるダメージ | `+%/×` | `give_damage` | `percent_max` | 完全一致のみ |
| 被ダメ | `±%` | `damage_taken` | `percent_max` | |
| 与ダメ回復 | `+%` | `damage_recovery` | `percent_max` | |

#### 速度・隙系（分離）

| 旧type | 旧unit | → Reborn Stat | → BuffMode | 備考 |
|--------|--------|---------------|------------|------|
| 速度 | `±%` | `attack_speed` | `percent_max` | |
| 隙 | `+%` | `attack_gap` | `percent_reduction` | 短縮 |
| 隙 | `-%` | `attack_gap` | `percent_max` | 増加 |

#### 気・計略系

**注意**: 気バフは種類ごとに専用のStatを使用します（v2.1で変更）

| パターン | → Reborn Stat | BuffMode | 備考 |
|---------|---------------|----------|------|
| 自然に気が増加 | `cost` | `percent_max` | 40%（増加）/70%（大きく増加） |
| 徐々に気がN増加 | `cost_gradual` | `flat_sum` | 時間経過・巨大化毎など |
| 敵撃破時の獲得気 | `cost_enemy_defeat` | `flat_sum` | 気(牛) - テキストに「敵」含む |
| 撃破獲得気N増加 | `cost_defeat_bonus` | `flat_sum` | 気(ノビ) - テキストに「敵」含まない |
| 巨大化気N%軽減 | `cost_giant` | `percent_reduction` | %軽減は巨大化気のみ対象 |
| 消費気がN軽減 | `cost_giant` | `flat_sum` | 固定値は両方対象 |
| 計略消費気 | `cost_strategy` | `flat_sum` | |
| 計略短縮 | `strategy_cooldown` | `percent_reduction` | |

#### その他

| 旧type | 旧unit | → Reborn Stat | → BuffMode | 備考 |
|--------|--------|---------------|------------|------|
| 射程割合 | `±%` | `range` | `percent_max` | |
| 射程固定 | `±` | `range` | `flat_sum` | |
| 対象数 | `±` | `target_count` | `flat_sum` | |
| 攻撃回数 | `+` | `attack_count` | `flat_sum` | |
| 回復 | `+` | `recovery` | `flat_sum` | |
| 回復割合 | `×` | `recovery` | `percent_max` | 倍率変換 |
| 移動低下 | `-%` | `enemy_movement` | `percent_max` | |
| 移動後退 | `+` | `enemy_knockback` | `flat_sum` | マス数 |

---

## 3. アーキテクチャ

### 3.1 ディレクトリ構成

```
src/
├── core/                      # ピュアロジック（React非依存）
│   ├── types/
│   │   └── index.ts           # 型定義（完全版）
│   ├── parser/
│   │   ├── buffParser.ts      # 旧buffParser.jsのTS移植
│   │   ├── patterns.ts        # 正規表現パターン定義
│   │   ├── converter.ts       # 旧形式→Reborn形式変換
│   │   └── __tests__/
│   │       └── buffParser.test.ts
│   ├── logic/
│   │   ├── buffs.ts           # バフ計算ロジック
│   │   ├── damageCalculator.ts # ダメージ計算
│   │   └── __tests__/
│   │       ├── buffs.test.ts
│   │       └── damageCalculator.test.ts
│   ├── wiki/                  # Wikiパース機能
│   │   ├── fetcher.ts         # HTML取得
│   │   ├── parser.ts          # HTML→RawData
│   │   ├── analyzer.ts        # RawData→Character
│   │   └── types.ts           # Wiki関連型定義
│   ├── data/
│   │   ├── weaponMapping.ts   # 武器種マッピング
│   │   └── weaponFrames.ts    # 武器種フレームデータ
│   ├── conditions.ts          # 条件タグ判定ロジック
│   └── mock/
│       └── data.ts            # テスト用データ

├── ui/                        # Reactコンポーネント
│   ├── components/
│   │   ├── FormationGrid.tsx
│   │   ├── BuffMatrix.tsx
│   │   ├── BuffStackBar.tsx
│   │   ├── WikiImporter.tsx   # Wiki取り込みモーダル
│   │   ├── WikiImport.tsx     # Wiki取り込みUI
│   │   └── AttackerAnalysisModal.tsx
│   ├── hooks/
│   │   └── useFormation.ts
│   └── contexts/
│       └── FormationContext.tsx

├── App.tsx
└── main.tsx
```

### 3.2 データフロー

```
[Wiki URL]
    ↓
[Fetcher] ─────────────────────────────────────────────┐
    ↓ HTML                                              │
[Wiki Parser] (parseWikiHTML)                          │
    ↓ RawData { skillsText, strategiesText }           │
[Buff Parser] (parseSkillLine) ← 旧buffParser移植       │
    ↓ ParsedBuff[]                                      │
[Converter] (convertToRebornBuff)                      │
    ↓ Buff[]                                            │
[Character Builder]                                    │
    ↓ Character                                         │
[Formation State] ←─────────────────────────────────────┘
    ↓
[calcBuffMatrix] (ピュア関数)
    ↓ BuffMatrixResult
[React Components] → 表示
```

---

## 4. 移植計画

### Phase 1: buffParserの移植（優先度：最高）

#### 4.1 移植対象（buffParser.js より）

1. **正規表現パターン** (~50個)
   - 攻撃バフ、防御バフ、射程バフ...
   - 条件付きバフ（巨大化時、〇〇属性の城娘...）

2. **parseSkillLine 関数**
   - 特技テキスト → ParsedBuff[] への変換

3. **weaponMapping**
   - 武器種 → { range, type, placement }

4. **targetMapping / legacyTargetMapping**
   - ターゲット表記の正規化

#### 4.2 移植手順

```
Step 1: types/index.ts
  - 完全な型定義を作成
  - Stat, BuffMode, Buff インターフェースを定義
  - サブタイプ（CostBuffType等）を定義

Step 2: patterns.ts
  - 正規表現パターンをTypeScriptで定義
  - パターンごとに対応するStat/BuffModeを明記

Step 3: buffParser.ts
  - parseSkillLine を移植
  - 戻り値型を ParsedBuff として定義

Step 4: converter.ts
  - ParsedBuff → Buff への変換関数
  - 旧表記→新表記のマッピング
  - 倍率→%変換、効果重複検出、鼓舞処理等

Step 5: テスト作成
  - 既知の特技テキストで期待値を確認
  - 「攻撃が30%上昇」→ { stat: 'attack', value: 30, mode: 'percent_max' }
  - 「自身の攻撃の30%を加算」→ { stat: 'inspire', inspireSourceStat: 'attack', value: 30 }
```

### Phase 2: ロジック完成（buffs.ts）

1. `isBuffApplicable` の射程判定実装
2. 条件タグ（ConditionTag）の判定ロジック
3. 巨大化バフの処理
4. 動的バフの識別と除外

### Phase 3: テスト充実

1. 各パターンの単体テスト
2. 複合バフのテスト
3. 実際のキャラデータでの結合テスト
4. 動的バフの検出テスト

### Phase 4: UI実装

1. BuffStackBar コンポーネント
2. ReferenceGauge コンポーネント
3. FormationSummary コンポーネント（新規）
4. AttackerAnalysis 画面
5. 動的バフセクションの表示

---

## 5. 型定義の拡張案

### 5.1 ParsedBuff（パーサー出力型）

buffParser.tsが出力する中間型です。この型は旧buffParser.jsの出力形式に近く、後で標準的なBuff型に変換されます。

```typescript
// parser/types.ts
export interface ParsedBuff {
  type: string;          // '攻撃割合', '防御固定' など
  target: string;        // '自身', '射程内', '城娘' など
  value: number;
  unit: string;          // '+%', '+' など
  isSpecial: boolean;    // 特殊条件付きか
  hasCondition: boolean; // 条件付きか
  conditionText?: string; // 条件の説明テキスト
  note: string;
  rawText: string;       // 元のテキスト（デバッグ用）
}
```

### 5.2 WeaponInfo

武器種の情報を表す型です。**日本語ベースの表記を採用**します。

```typescript
// data/weaponMapping.ts
export interface WeaponInfo {
  range: '近' | '遠';
  type: '物' | '術';
  placement: '近' | '遠' | '遠近';
}

export const weaponMapping: Record<string, WeaponInfo> = {
  "弓": { range: "遠", type: "物", placement: "遠" },
  "鉄砲": { range: "遠", type: "物", placement: "遠" },
  "石弓": { range: "遠", type: "物", placement: "遠" },
  "投剣": { range: "遠", type: "物", placement: "遠近" },
  "軍船": { range: "遠", type: "物", placement: "遠近" },
  "槍": { range: "近", type: "物", placement: "近" },
  "刀": { range: "近", type: "物", placement: "近" },
  "盾": { range: "近", type: "物", placement: "近" },
  "ランス": { range: "近", type: "物", placement: "近" },
  "双剣": { range: "近", type: "物", placement: "近" },
  "拳": { range: "近", type: "物", placement: "近" },
  "鞭": { range: "近", type: "物", placement: "遠近" },
  "茶器": { range: "近", type: "物", placement: "遠近" },
  "歌舞": { range: "遠", type: "術", placement: "遠" },
  "本": { range: "遠", type: "術", placement: "遠" },
  "砲術": { range: "遠", type: "術", placement: "遠" },
  "鈴": { range: "遠", type: "術", placement: "遠" },
  "杖": { range: "遠", type: "術", placement: "遠" },
  "札": { range: "遠", type: "術", placement: "遠" },
  "大砲": { range: "遠", type: "物", placement: "遠近" },
  "陣貝": { range: "遠", type: "術", placement: "遠近" }
};
```

**採用理由**:
- コードを読む際の直感性が高い
- 旧版との互換性を保つ
- TypeScriptの型チェックの恩恵は同様に受けられる

### 5.3 ConditionTag の拡張

バフの適用条件を表すタグです。詳細な設計は `CONDITION_TAG_SPEC.md` を参照してください。

```typescript
export type ConditionTag =
  // 武器系（優先度：中）
  | 'melee' | 'ranged'
  | 'physical' | 'magical'
  // 属性系（優先度：中）
  | 'water' | 'plain' | 'mountain' | 'plain_mountain' | 'hell'
  // HP条件（優先度：高）
  | 'hp_above_50' | 'hp_below_50'
  | 'hp_above_70' | 'hp_below_30' | 'hp_full'
  | 'hp_dependent'           // HP依存（HPに比例して効果変動）
  // 巨大化段階条件（優先度：中〜低）
  | 'giant_1_plus' | 'giant_2_plus' | 'giant_3_plus' | 'giant_4_plus' | 'giant_5'
  // 季節系（優先度：中）
  | 'summer' | 'kenran' | 'halloween' | 'school'
  | 'christmas' | 'new_year' | 'moon_viewing' | 'bride'
  // 対象種別（優先度：中〜低）
  | 'castle_girl' | 'ambush' | 'lord'
  // 敵種別（優先度：中）
  | 'flying_enemy' | 'ground_enemy' | 'boss_enemy'
  // 特殊条件（優先度：低〜中）
  | 'same_weapon' | 'different_weapon'
  | 'night_battle' | 'continuous_deploy'
  | 'on_water'               // 水上マップ
  | 'on_placement'           // 配置時
  | 'exclude_self';          // 自身を除く
```

### 5.4 鼓舞（Inspire）の仕様

鼓舞は、自身のステータスを参照して味方に加算する特殊なバフです。通常のバフとは計算方法が異なります。

#### 基本構造

```typescript
interface InspireBuff extends Buff {
  stat: 'inspire';
  mode: 'percent_max';
  value: number;                    // 何%を加算するか
  inspireSourceStat: Stat;          // どのステータスを参照するか
  target: 'range' | 'all';
}
```

#### 例：「自身の攻撃の30%を射程内に加算」

```typescript
{
  id: 'inspire-001',
  stat: 'inspire',
  mode: 'percent_max',
  value: 30,
  inspireSourceStat: 'attack',      // 自身の攻撃力を参照
  target: 'range',
  source: 'self_skill',
  isActive: true
}
```

#### 複数ステータス参照

「自身の攻撃と防御の30%を射程内に加算」のような場合、別々のBuffとして生成します。

```typescript
// 攻撃の鼓舞
{
  stat: 'inspire',
  inspireSourceStat: 'attack',
  value: 30,
  target: 'range'
}

// 防御の鼓舞
{
  stat: 'inspire',
  inspireSourceStat: 'defense',
  value: 30,
  target: 'range'
}
```

#### 計算方法（概念）

詳細なダメージ計算式は `DAMAGE_CALCULATOR_SPEC.md` を参照してください。

```
1. キャラAの基礎攻撃力 = 1000
2. キャラAに攻撃+50%バフ = 1000 × 1.5 = 1500
3. キャラAは「自身の攻撃の30%を鼓舞」を持つ
4. 鼓舞で味方に加算される値 = 1500 × 0.3 = 450
5. 味方キャラBの攻撃力 = 800 + 450 = 1250
```

### 5.5 効果重複（Duplicate）の仕様

「効果重複」が付いたバフは、通常のバフとは異なる計算タイミングで処理されます。

#### フラグ付与

```typescript
{
  stat: 'enemy_defense',
  mode: 'percent_max',
  value: 5,
  isDuplicate: true,     // 効果重複フラグ
  target: 'range'
}
```

#### UI表示

効果重複バフは、バッジで視覚的に区別します。

```
┌──────────────────────┐
│ 射程内/敵防御-5%        │
│ [重複] ← バッジ表示    │
└──────────────────────┘
```

#### 計算方法（概念）

詳細な計算式は `DAMAGE_CALCULATOR_SPEC.md` を参照してください。

```
通常バフフェーズ:
  攻撃力 = (基礎攻撃力 × 割合バフ) + 固定バフ

効果重複バフフェーズ:
  攻撃力 = 攻撃力 × (1 + 効果重複バフ%)

ダメージ乗算フェーズ:
  ダメージ = 攻撃力 × 与ダメ × 被ダメ × ...
```

### 5.6 動的バフ（Dynamic Buffs）の仕様

編成構築時に具体的な値が確定できないバフを「動的バフ」として分類します。

#### 2つのカテゴリ

```typescript
// 編成依存：伏兵数、味方数など（想定値を示せる）
{
  isDynamic: true,
  dynamicCategory: 'formation',
  dynamicType: 'per_ally_other',
  unitValue: 150,  // 味方1体につき+150
  note: '味方1体につき攻撃+150（最大7体：+1050）'
}

// 戦闘状況依存：敵数など（予測不可能）
{
  isDynamic: true,
  dynamicCategory: 'combat_situation',
  dynamicType: 'per_enemy_in_range',
  unitValue: 12,  // 敵1体につき+12%
  note: '射程内敵1体毎に防御+12%'
}
```

#### UI設計

動的バフは「動的バフ（参考情報）」セクションに別枠表示します。

```
【動的バフ（参考情報）】
⚠️ 実際の効果は戦闘条件によって変動します

キャラC（自身）:
  🔄 防御 +12%/敵（射程内の敵1体毎）
     - 5体：+60%
     - 10体：+120%
     - 巨大化5のみ、重複あり
```

詳細は `PARSING_RULES.md` のパターン21を参照してください。

---

## 6. テスト戦略

### 6.1 単体テスト（パーサー）

```typescript
// parser/__tests__/buffParser.test.ts
describe('parseSkillLine', () => {
  test.each([
    ['攻撃が30%上昇', { stat: 'attack', value: 30, mode: 'percent_max' }],
    ['攻撃が50上昇', { stat: 'attack', value: 50, mode: 'flat_sum' }],
    ['射程内の城娘の攻撃が20%上昇', { stat: 'attack', value: 20, target: 'range' }],
    ['自身の攻撃速度が15%上昇', { stat: 'attack_speed', value: 15 }],
    ['隙が30%短縮', { stat: 'attack_gap', mode: 'percent_reduction', value: 30 }],
    ['自身の攻撃の30%を射程内に加算', { 
      stat: 'inspire', 
      inspireSourceStat: 'attack', 
      value: 30,
      target: 'range'
    }],
  ])('"%s" → %o', (input, expected) => {
    const result = parseSkillLine(input);
    expect(result[0]).toMatchObject(expected);
  });
});
```

### 6.2 結合テスト（編成計算）

```typescript
describe('calcBuffMatrix with real data', () => {
  test('江戸城の全体バフが全員に適用される', () => {
    // ...
  });
  
  test('鼓舞バフが正しく計算される', () => {
    // ...
  });
  
  test('効果重複バフが別フェーズで処理される', () => {
    // ...
  });
  
  test('動的バフが主計算から除外される', () => {
    // ...
  });
});
```

---

## 7. 次のアクション

### 今すぐやること
1. ✅ 設計書作成（本ドキュメント）
2. ✅ 型変換マッピング仕様書作成（TYPE_CONVERSION_MAPPING.md）
3. ✅ CONDITION_TAG仕様書作成（CONDITION_TAG_SPEC.md）
4. ✅ パーシングルール集作成（PARSING_RULES.md）
5. ✅ ダメージ計算仕様書作成（DAMAGE_CALCULATOR_SPEC.md）
6. ✅ patterns.ts の作成
7. ✅ buffParser.ts の移植
8. 🔲 UI設計の詳細化

### 重要なドキュメント

- **ARCHITECTURE.md（本ドキュメント）** - プロジェクト全体の設計書
- **TYPE_CONVERSION_MAPPING.md** - 型変換の完全な仕様
- **CONDITION_TAG_SPEC.md** - 条件タグの完全な定義
- **PARSING_RULES.md** - Wikiテキスト解析のコア仕様（パターン1〜10）
- **PARSING_RULES_ADVANCED.md** - 高度なパターン（パターン11〜23）
- **WIKI_PARSER_GUIDE.md** - Wikiパーサー実装知見とバージョン履歴
- **DAMAGE_CALCULATOR_SPEC.md** - ダメージ計算式の詳細仕様

---

## 8. 参考：UI設計（仕様書より）

### BuffMatrix のセル表示
```
┌─────────────────────────────┐
│ ● ◯ ●  (誰からバフを受けているか) │
│ [■■□□] 30% (自20% + 味方10%)    │
└─────────────────────────────┘
```

### 鼓舞バフの表示
```
┌──────────────────────┐
│ 🎺 射程内/鼓舞（攻撃）   │
│    自身の攻撃の30%を加算 │
└──────────────────────┘
```

### 効果重複バフの表示
```
┌──────────────────────┐
│ 射程内/敵防御-5%        │
│ [重複] ← バッジ        │
└──────────────────────┘
```

### 動的バフの表示
```
┌──────────────────────┐
│ ⚠️ 攻撃 +150/味方     │
│    動的（編成依存）    │
│    7体：+1050         │
│    10体：+1500        │
└──────────────────────┘
```

### 伏兵バフの表示
```
┌──────────────────────┐
│ ⚡ 攻撃 +120          │
│    伏兵の射程内       │
│    配置位置に依存     │
└──────────────────────┘
```

### リファレンスゲージ
```
[====|=====|===] 75%
 赤(不足) 緑(適正) 青(過剰)
```

### 編成サマリー（新規）
```
┌──────────────────────┐
│ 【編成構成】          │
│ 近接: 5体  遠隔: 3体  │
│ 水: 2  平: 3  山: 2   │
│                      │
│ ⚠️ 平城3体制限超過    │
└──────────────────────┘
```

---

## 9. Wikiパーサーの実装知見

### 9.1 特殊能力 vs 特殊攻撃

Wikiには「特殊能力」と「特殊攻撃」という別々のセクションが存在します。

- **特殊能力**: パッシブスキル（気(牛)などが含まれる）
- **特殊攻撃**: アクティブスキル（ストック消費攻撃など）

パーサーは特殊攻撃セクションを除外し、特殊能力のみを抽出します。

### 9.2 効果重複の位置ベース判定

「(同種効果と重複)」のような表記は、**その表記より前のバフにのみ適用**されます。

```
例: 攻撃1.2倍(同種効果と重複)被ダメージ1.5倍(自分のみ)
→ 攻撃1.2倍: isDuplicate = true
→ 被ダメージ1.5倍: isDuplicate = false（効果重複より後ろなので）
```

### 9.3 並列表記の展開順序

「攻撃と攻撃速度」のような並列表記を展開する際、**長いパターンを先にマッチ**させます。

```typescript
const statPattern = '攻撃速度|移動速度|攻撃(?:力)?|防御(?:力)?|射程|...';
// 「攻撃速度」を「攻撃」より先に定義することで誤マッチを防止
```

### 9.4 徐々気パターンの厳格化

以下のような誤検知を防ぐため、徐々気パターンは厳格に定義します:

```
❌ 誤検知: 「時間経過でも蓄積...獲得気が2増加」
   → 「時間経過で」と「獲得気が2」が離れていてもマッチしてしまう

✓ 正しいパターン: 時間経過で気が(?:徐々に)?(\d+)(?:増加|回復)
   → 「時間経過で気が」が直接続く場合のみマッチ
```

---

## 10. 変更履歴

### v2.1 (2025-12-14)
- 気バフを種類ごとに専用Statに分離（cost_gradual, cost_giant, cost_enemy_defeat, cost_defeat_bonus, cost_strategy）
- 敵の被ダメ/与ダメ関連Stat追加（enemy_damage_taken, enemy_damage_dealt）
- enemy_knockback を enemy_retreat に変更
- Wikiパーサーの実装知見セクションを追加
- 効果重複の位置ベース判定仕様を追加

### v2.0 (2025-12-07)
- Targetの6種類を明記（self, ally, range, all, field, out_of_range）
- TARGET_NORMALIZATION.mdの内容を統合
- weaponMappingを日本語ベース（'近', '遠', '物', '術'）に変更
- 動的バフの詳細仕様を追加
- 伏兵バフ（requiresAmbush）の仕様を追加
- 計略範囲バフの扱いを明記
- 手動編集サポート（confidence）を追加
- 編成サマリー機能を追加
- DAMAGE_CALCULATOR_SPEC.mdへの参照を追加

### v1.0 (2025-12-06)
- 初版作成
- 基本的なデータモデルとアーキテクチャを定義

---

*Last Updated: 2025-12-14*
*Version: 2.1*
