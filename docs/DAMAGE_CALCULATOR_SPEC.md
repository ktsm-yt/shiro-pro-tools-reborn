...
**すべての倍率は個別に乗算**される（加算ではない）。複数あっても加算されず、それぞれが掛け合わされる。

倍率の種類（stat: `give_damage`）:
- 「攻撃の◯倍のダメージ」
- 「与えるダメージが◯倍」（フル表記）
- 「◯倍のダメージを与える」
- 「直撃ボーナス」（下記参照）
...
直撃ボーナスは stat: `critical_bonus` として扱われ、Phase 2で乗算されます。

```
計算式: 直撃ボーナスX% = (100 + X) / 100 倍
```

- **ベース直撃ボーナス**: 50%（城娘のデフォルト）
- **absolute_set**: 「直撃ボーナスが300%に上昇」→ (100+300)/100 = 4.0倍
- **percent_max**: 「直撃ボーナスが120%上昇」→ ベース50% + 120% = 170% = (100+170)/100 = 2.70倍

例：
- 直撃ボーナス50%（デフォルト）→ (100+50)/100 = 1.5倍
- 直撃ボーナス170%（120%上昇後）→ (100+170)/100 = 2.7倍

**重要:** 「与えるダメージ」（フル表記）と「与ダメ」（略称）は異なるルール:
- `give_damage`（与えるダメージ）: **Phase 2で乗算** - 複数あれば全て個別に乗算
- `damage_dealt`（与ダメ）: **Phase 4で最大値のみ** - 複数あっても最大値のみ適用
